...
function CacheInsights() {
    ...
    const hitRateColor = cacheStatus.hitRate >= 70 ? 'text-green-600' : cacheStatus.hitRate >= 40 ? 'text-orange-600' : 'text-red-600'
    const hitRateBgColor = cacheStatus.hitRate >= 70 ? 'bg-green-100' : cacheStatus.hitRate >= 40 ? 'bg-orange-100' : 'bg-red-100'
    
    const efficiency = cacheStatus.hitRate
    const speedBoost = Math.round(cacheStatus.hitRate * 0.3) // Estimate speed boost multiplier
    const performanceImpact = cacheStatus.hitRate >= 70 ? 'Excellent boost' : cacheStatus.hitRate >= 40 ? 'Good boost' : 'Needs optimization'
    ...
    const fetchCacheData = async () => {
        ...
        try {
            ...
            setCacheStatus({
                size: cacheData.total_entries || cacheData.size || 0,
                hitRate: (cacheData.hit_rate || cacheData.hitRate || 0) * 100, // Convert to percentage if needed
                storageUsed: cacheData.storage_used || cacheData.storageUsed || '0 MB'
            })
    ...
    const handleOptimize = async () => {
        ...
        try {
            // This could call an optimization endpoint if available
            await api.getCacheStatus() // Refresh data
                    ...
                    <div className='relative w-16 h-16 mx-auto mb-2'>
                        <svg className='w-16 h-16 transform -rotate-90' viewBox='0 0 64 64'>
                            ...
                            <motion.circle
                                ...
                                r='28'
                                fill='none'
                                stroke={cacheStatus.hitRate >= 70 ? '#10b981' : cacheStatus.hitRate >= 40 ? '#f59e0b' : '#ef4444'}
                                strokeWidth='6'
                                strokeLinecap='round'
                        ...
                        <div className='absolute inset-0 flex items-center justify-center'>
                            <span className='text-lg font-bold text-gray-800'>{Math.round(cacheStatus.hitRate)}%</span>
                        </div>
                    </div>
                ...
                <div className='grid grid-cols-2 gap-3'>
                    <div className='bg-gray-50 rounded-lg p-3 text-center'>
                        ...
                        <p className='text-sm font-bold text-gray-800'>
                            {isLoading ? '...' : cacheStatus.size.toLocaleString()}
                        </p>
                    ...
                    <div className='bg-gray-50 rounded-lg p-3 text-center'>
                        ...
                        <p className='text-sm font-bold text-gray-800'>
                            {isLoading ? '...' : cacheStatus.storageUsed}
                        </p>
