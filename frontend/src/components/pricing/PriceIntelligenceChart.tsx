  ...
  const chartData = React.useMemo(() => {
    ...
    if (currentPrices && Array.isArray(currentPrices)) {
      ...
      currentPrices.forEach((priceInfo: any) => {
        ...
        // Use priceDate if available, fall back to lastUpdated
        const actualPriceDate = priceInfo.priceDate || priceInfo.lastUpdated;
        const priceDate = new Date(actualPriceDate).toISOString().split('T')[0];
        
        if (!dataMap.has(priceDate)) {
          dataMap.set(priceDate, { 
            date: priceDate, 
            timestamp: new Date(priceDate).getTime() 
          });
        }
        const dataPoint = dataMap.get(priceDate);
        dataPoint[supermarket] = priceInfo.price;
      });
    }
    
    // Only show real data - no sample generation
    if (dataMap.size === 0) {
      console.log('⚠️ No real data available for chart');
      return [];
    }
    ...
    return (
      <div className={`${className} bg-white border rounded-xl p-6`}>
        <h4 className="text-lg font-semibold text-gray-900 mb-4">
          Interactive Price Intelligence - {productName}
        </h4>
        ...
        <LineChart data={chartData}>
          ...
          {availableSupermarkets.map((supermarket) => (
            <Line
              ...
              strokeWidth={3}
              dot={{ r: 5, strokeWidth: 2, fill: colors[supermarket] || '#6366f1' }}
              activeDot={{ r: 8, strokeWidth: 3, fill: colors[supermarket] || '#6366f1' }}
              connectNulls={true}
              strokeDasharray={supermarket === 'keells' ? '0' : '0'}
            />
          ))}
        </LineChart>
      </ResponsiveContainer>
            ...
            <div key={supermarket} className="bg-gray-50 rounded-lg p-4">
              ...
              <div className="space-y-1 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">Rs {avgPrice.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
