"""Integration tests for the OpenRouter handler.

These tests invoke real API calls and require the OPENROUTER_API_KEY
...
# Add the backend directory and project root to the path.
...
def test_openrouter_real_classification(handler, allowed_models):
    """Test real product classification with the OpenRouter API for each allowed model."""
...
Return JSON: {"category": "Electronics", "sub_category": "Smartphones", "brand": "Samsung"}"""
    ...
    for test_model in allowed_models:
        ...
        # Classify with the selected model.
...
def test_openrouter_free_models(handler, allowed_models):
    ...
    # Test with up to two allowed models.
...
def test_openrouter_response_time(handler, allowed_models):
    """Test that the API responds within an acceptable time."""
    if not allowed_models:
        pytest.skip("No allowed OpenRouter models configured")
...
def test_openrouter_error_handling(handler, allowed_models):
    ...
    try:
        ...
        # Expected: invalid model should raise an error.
