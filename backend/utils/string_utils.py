...
def clean_product_name(name):
    """
    Clean and format product names for display.
    ...
    # Remove repeated brand names.
    words = name.split()
    if len(words) >= 2 and words[0].lower() == words[1].lower():
        # Remove the duplicate brand name at the start.
        words = words[1:]
        name = ' '.join(words)
    
    # Normalize case.
    # Keep acronyms uppercase and title case other words.
    words = []
    for word in name.split():
        if len(word) <= 3 and word.isupper():
            # Keep short uppercase words (e.g., ML, KG).
            words.append(word)
