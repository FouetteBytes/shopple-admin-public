...
This test suite runs all 9 Keells crawlers and generates a comprehensive report.
Uses the base crawler to test all categories with their respective URLs.

Benefits:
- Single test file for all crawlers
- Tests base crawler implementation with real data
- Generates comprehensive metrics for each category
- Perfect for CI/CD with detailed Slack reporting
"""
...
class CrawlerTestResults:
    ...
    def get_summary(self) -> str:
        """Get text summary of all results"""
...
if PYTEST_AVAILABLE:
    class TestAllKeellsCrawlers:
        ...
        def crawler_config(self):
            """Get crawler configuration with MAX_ITEMS limit for tests"""
            max_items = os.getenv('MAX_ITEMS', '10')  # Default to 10 for faster tests
            return {
        ...
        async def test_base_crawler_initialization(self):
            """Test that base crawler can be imported and initialized"""
            try:
        ...
        async def test_all_keells_crawlers(self, crawler_config, test_output_dir, results_tracker):
            """
            Test all 9 Keells crawlers sequentially
            
            This is the main test that runs all crawlers and collects metrics.
            Results are stored in results_tracker for reporting.
            """
            ...
            for category_key, config in KEELLS_CRAWLERS.items():
                ...
                try:
                    ...
                    # Extract sample product names (first 3)
                    samples = [
            ...
            # ALWAYS save results to JSON file for GitHub Actions (even if some tests failed)
            try:
